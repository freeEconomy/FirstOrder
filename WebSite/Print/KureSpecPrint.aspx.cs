...
public partial class Print_KureSpecPrint : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        sb.AppendLine(" ,(select Bjhd2_SongJangNox from " + preVal + "BJHD2 where isnull(Bjhd2_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd2_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd2_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd2_Sample,'') = isnull(a.Bjhd_Sample,'')) as Bjhd2_SongJangNox ");
        ...
        string qry = " select (select count(*) from " + preVal + "DNGA where Dnga_StyleNox=a.Blju_StyleNox) as styleCnt,* from " + preVal + "BLJU a where Blju_Date = '" + blju_date + "' and Blju_Times = '" + blju_times + "' and Blju_MainBuyer = '" + blju_mainbuyer + "' and Blju_Sample = '" + blju_sample + "' order by Blju_Line ";
        ...
        {
            int styleCnt = StCommon.ToInt(ds.Tables[0].Rows[i]["styleCnt"].ToString(), 0);
            ...
            {
                dt.Rows.Add(listNo, style, "", "", "", "", GetAmountFormat(justAmount));
                listNo = listNo + 1;
                netAmount = netAmount + justAmount;
            }
            else
        ...
        qry = " UPDATE " + preVal + "BJHD SET Bjhd_KureMyung_Print = 'Y' " + whereQry;
