...
namespace FirstOrder.Data
{
    public class OrderData_common
    {
        ...
        public void InsertHead()
        {
            ...
            sb.Append(" select * from " + preVal + "BJHD where ");
            ...
            if (ds.Tables[0].Rows.Count == 0)
            {
                ...
                sb.Append(" INSERT INTO " + preVal + "BJHD (Bjhd_Date,Bjhd_Times,Bjhd_MainBuyer,Bjhd_Sample,Bjhd_BaeSong,Bjhd_BaeSongName,Bjhd_NetAmount,Bjhd_VatAmount,Bjhd_HapAmount,Bjhd_Remark ");
                ...
                if (m_ssawon != "") {
                    ...
                    whis.InsertWork("발주", "헤더입력(세션만료)", cmd);
                }
            }
            else
            {
        ...
        public void UpdateHead()
        {
            ...
            sb.Append(" UPDATE " + preVal + "BJHD SET ");
            ...
            if (m_msawon != "") {
                ...
                whis.InsertWork("발주", "헤더수정(세션만료)", cmd);
            }
        }
        ...
        public void InsertData()
        {
            ...
            sb.Append(" INSERT INTO " + preVal + "BLJU (Blju_Date,Blju_Times,Blju_MainBuyer,Blju_Sample,Blju_Line,Blju_StyleNox,Blju_Qty01,Blju_Qty02,Blju_Qty03,Blju_Qty04,Blju_Qty05,Blju_Qty06,Blju_Qty07,Blju_Qty08,Blju_Qty09 ");
            ...
            sb.Append(" ,Blju_DngaLowPrice,Blju_DngaJustPrice,Blju_DngaBigSizePrice,Blju_DngaSizeBoxQty,Blju_DngaBoxQty) ");
        ...
        public void UpdateData()
        {
            ...
            sb.Append(" UPDATE " + preVal + "BLJU SET ");
        ...
        public int MaxLine()
        {
            ...
            sb.Append(" select isnull(max([Blju_Line]) + 1,1) as maxLine from " + preVal + "BLJU ");
        ...
        public void UploadPicture(byte[] imgData, string preVal, string product)
        {
            StDataCommon stData = new StDataCommon();

            string qry = " select * FROM " + preVal + "JEPG where Jepg_StyleNox = '" + product + "' ";
            DataSet ds = stData.GetDataSet(qry);

            if (ds.Tables[0].Rows.Count == 0)
            {
                qry = " insert into " + preVal + "JEPG(Jepg_StyleNox,Jepg_Image) values (@Jepg_StyleNox,@Jepg_Image); ";
                DbCommand cmd = db.GetSqlStringCommand(qry);

                db.AddInParameter(cmd, "Jepg_StyleNox", SqlDbType.VarChar, product);
                db.AddInParameter(cmd, "Jepg_Image", SqlDbType.Image, imgData);

                db.ExecuteNonQuery(cmd);

                whis.InsertWork("스타일", "사진등록", cmd);
            }
            else
            {
                qry = " update " + preVal + "JEPG set Jepg_Image = @Jepg_Image where Jepg_StyleNox = @Jepg_StyleNox ";
                DbCommand cmd = db.GetSqlStringCommand(qry);
                ...
                whis.InsertWork("스타일", "사진수정", cmd);
            }
        }

        public static string GetStopCheck(string preVal)
        {
            string result = "";
            StDataCommon stData = new StDataCommon();

            string code = (preVal == "tbl") ? "BLJU_MARKER" : "BLJU_TOPCLASS";

            string qry = " select * from gblCOMMON where Common_key  = 'T0701' and Common_Code = '" + code + "' ";
            DataSet ds = stData.GetDataSet(qry);

            if (ds.Tables[0].Rows.Count > 0)
            {
                if (ds.Tables[0].Rows[0]["common_remark3"].ToString() == "-999")
                    result = "죄송합니다.....기 공지해 드린 것과 같이\\n\\n본사 사정에 의해 '발주의뢰'를 일시 중지합니다. !!!";
            }
