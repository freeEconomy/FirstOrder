...
public class WebService_abl : System.Web.Services.WebService
{
    public WebService_abl()
    {
    ...
    public string GetProduct(string paramDate, string paramTime, string paramKure, string searchValue)
    {
        ...
        qry = " select * from View_ablJEGO_Summary where Jego_StyleNox like '%" + searchValue + "%' ";
        ...
        using (IDataReader dr = stData.GetDataReader(qry))
        {
            if (dr.Read())
            {
                ...
                qry = " select * from ablBLJU where Blju_Date = '" + paramDate + "' and Blju_Times = '" + paramTime + "' and Blju_MainBuyer = '" + paramKure + "' and Blju_StyleNox = '" + dr["Jego_StyleNox"].ToString() + "' ";
                ...
                qry = " select Dnga_MainName,Dnga_SubName,Dnga_SpecColor,isnull(Dnga_LowPrice,0) as Dnga_LowPrice,isnull(Dnga_JustPrice,0) as Dnga_JustPrice,isnull(Dnga_BigSizePrice,0) as Dnga_BigSizePrice,Dnga_SizeBoxQty,Dnga_BoxQty from ablDNGA where Dnga_StyleNox = '" + dr["Jego_StyleNox"].ToString() + "' ";
                ...
                qry = "SELECT * FROM ablJEPG WHERE Jepg_StyleNox='" + dr["Jego_StyleNox"].ToString() + "'";
                ds = stData.GetDataSet(qry);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    try
                ...
                while (dr.Read())
                {
                    ...
                    qry = " select isnull(Dnga_JustPrice,0) as Dnga_JustPrice,isnull(Dnga_LowPrice,0) as Dnga_LowPrice,isnull(Dnga_BoxQty,0) as Dnga_BoxQty,Dnga_StyleNox,Dnga_MainName,Dnga_SubName,Dnga_SpecColor from ablDNGA where Dnga_StyleNox = '" + dr["Jego_StyleNox"].ToString() + "' ";
                    ds = stData.GetDataSet(qry);
