...
public partial class Page_Order_abl : System.Web.UI.Page
{
    ...
    private void BindView(string bljudate, string bljutime, string kurecode, string blju_sample)
    {
        ...
        string qry = " select (select kure_sangho from gblKURE where kure_code = a.Bjhd_MainBuyer) as Bjhd_MainBuyerNm,(select Bjhd2_SongJangNox from ablBJHD2 where isnull(Bjhd2_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd2_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd2_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd2_Sample,'') = isnull(a.Bjhd_Sample,'')) as Bjhd2_SongJangNox,* from ablBJHD a where Bjhd_Date = '" + bljudate + "' and Bjhd_MainBuyer = '" + kurecode + "' " + whereQry + " order by Bjhd_Times desc ";
