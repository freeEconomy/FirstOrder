...
public partial class Page_Order_abl : System.Web.UI.Page
{
    ...
    {
        this.isSizeNum.Text = MemberData.GetLoginSID("ablSizeNum");
        this.hidBljuMin.Value = StCommon.ToDouble(MemberData.GetLoginSID("ablBljuMin"), 0).ToString();
        ...
        if (!IsPostBack)
        {
            ...
            if (mode == "add")
            {
                if (param_date == "")
                {
                    ...
                    string qry = " select * from ablBJHD where Bjhd_Bonsa_Check in('0') and Bjhd_Date = '" + postbljudate + "' and Bjhd_MainBuyer = '" + kurecode + "' and Bjhd_CreateSawon = '" + MemberData.GetLoginSID("LoginID") + "' order by Bjhd_Times desc ";
    ...
    {
        // 당일 거래처의 발주내역이 있으면 가져올것(단, 본사처리가 안된 상태인 ablBJHD테이블 Bjhd_Bonsa_Check 필드가 '0'인 것만)
        string whereQry = "";
        whereQry = StCommon.MakeSearchQry("Bjhd_Times", bljutime, "S", whereQry);
        
        string qry = " select * from ablBJHD where Bjhd_Bonsa_Check in('0') and Bjhd_Date = '" + bljudate + "' and Bjhd_MainBuyer = '" + kurecode + "' " + whereQry + " and Bjhd_CreateSawon = '" + MemberData.GetLoginSID("LoginID") + "' order by Bjhd_Times desc ";
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            if (ds.Tables[0].Rows[0]["Bjhd_Bonsa_Check"].ToString() == "0")
            {
                ...
                qry = " select * from ablBLJU where Blju_Date = '" + bljudate + "' and Blju_Times = '" + bljutime + "' and Blju_MainBuyer = '" + kurecode + "' ";
    ...
    private void BindData(string kurename, string kurecode)
    {
        ...
        this.txtOrder13.Attributes.Add("onkeydown", "return fnNumberOnly(event);");
        this.txtOrder14.Attributes.Add("onkeydown", "return fnNumberOnly(event);");
        this.txtOrder15.Attributes.Add("onkeydown", "return fnNumberOnly(event);");
        this.txtOrder16.Attributes.Add("onkeydown", "return fnNumberOnly(event);");
        this.txtOrder17.Attributes.Add("onkeydown", "return fnNumberOnly(event);");
        ...
        this.txtOrder13.Attributes.Add("onkeypress", "return CheckProduct(event);");
        this.txtOrder14.Attributes.Add("onkeypress", "return CheckProduct(event);");
        this.txtOrder15.Attributes.Add("onkeypress", "return CheckProduct(event);");
        this.txtOrder16.Attributes.Add("onkeypress", "return CheckProduct(event);");
        this.txtOrder17.Attributes.Add("onkeypress", "return CheckProduct(event);");
    }
    ...
    private void BindDataList()
    {
        ...
        string qry = " SELECT isnull((select isnull(Dnga_SubName,'') + ', ' + isnull(Dnga_SpecColor,'') from ablDNGA where Dnga_StyleNox=a.Blju_StyleNox),'') as Blju_StyleName,* FROM ablBLJU a " + whereQry + " ORDER BY Blju_Line desc ";
        DataSet ds = stData.GetDataSet(qry);
        ...
        qry += " ,isnull(sum(blju_qty13),0) as q13,isnull(sum(blju_qty14),0) as q14,isnull(sum(blju_qty15),0) as q15,isnull(sum(blju_qty16),0) as q16,isnull(sum(blju_qty17),0) as q17 ";
        qry += " FROM ablBLJU a " + whereQry + " ";
        ...
        double sum13 = StCommon.ToDouble(dr["q13"].ToString(), 0);
        double sum14 = StCommon.ToDouble(dr["q14"].ToString(), 0);
        double sum15 = StCommon.ToDouble(dr["q15"].ToString(), 0);
        double sum16 = StCommon.ToDouble(dr["q16"].ToString(), 0);
        double sum17 = StCommon.ToDouble(dr["q17"].ToString(), 0);
        try
        {
            int k = 1;
            while (k <= 17)
    ...
    protected void lvMain_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            // 대리점은 ablBJHD테이블 Bjhd_Bonsa_Check 필드가 '0', '1'인 것만 처리할 수 있다. (나머지는 수정, 삭제를 할 수 없고 조회만 가능함)
            string qry = " SELECT * FROM ablBJHD where Bjhd_Bonsa_Check in('0','1') and bjhd_date = '" + drItemRow["blju_date"].ToString() + "' and bjhd_times = '" + drItemRow["blju_times"].ToString() + "' and bjhd_mainbuyer = '" + drItemRow["blju_mainbuyer"].ToString() + "' and bjhd_sample = '" + drItemRow["blju_sample"].ToString() + "' ";
            DataSet dsC = stData.GetDataSet(qry);
    ...
    protected void lvMain_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        // 현재 발주상태 체크
        string nowBonsaCheck = StCommon.GetBonsaCheck("abl", blju_mainbuyer, blju_date, blju_times);
        ...
        else
        {
            switch (e.CommandName)
            {
                ...
                case "subDelete":
                    ...
                    string qry = " DELETE FROM ablBLJU where blju_date = '" + blju_date + "' and blju_times = '" + blju_times + "' and blju_mainbuyer = '" + blju_mainbuyer + "' and blju_sample = '" + blju_sample + "' and blju_line = '" + blju_line + "' ";
                    stData.GetExecuteNonQry(qry);

                    whis.InsertWork("발주", "삭제", qry);

                    qry = " select * FROM ablBLJU where blju_date = '" + blju_date + "' and blju_times = '" + blju_times + "' and blju_mainbuyer = '" + blju_mainbuyer + "' and blju_sample = '" + blju_sample + "' ";
                    DataSet ds = stData.GetDataSet(qry);

                    if (ds.Tables[0].Rows.Count == 0) // 발주내역이 하나도 없으면 헤더도 삭제
                    {
                        qry = " DELETE FROM ablBJHD where bjhd_date = '" + blju_date + "' and bjhd_times = '" + blju_times + "' and bjhd_mainbuyer = '" + blju_mainbuyer + "' and bjhd_sample='" + blju_sample + "' ";
                        stData.GetExecuteNonQry(qry);

                        whis.InsertWork("발주헤더", "삭제", qry);

                        qry = " DELETE FROM ablBLJU where blju_date = '" + blju_date + "' and blju_times = '" + blju_times + "' and blju_mainbuyer = '" + blju_mainbuyer + "' and blju_sample='" + blju_sample + "' ";
                        stData.GetExecuteNonQry(qry);

                        whis.InsertWork("발주", "삭제", qry);

                        qry = " DELETE FROM ablBJHD0 where bjhd0_date = '" + blju_date + "' and bjhd0_times = '" + blju_times + "' and bjhd0_mainbuyer = '" + blju_mainbuyer + "' and bjhd0_sample='" + blju_sample + "' ";
                        stData.GetExecuteNonQry(qry);

                        whis.InsertWork("발주배송지", "삭제", qry);

                        qry = " DELETE FROM ablMESG where Mesg_Date = '" + blju_date + "' and Mesg_Times = '" + blju_times + "' and Mesg_MainBuyer = '" + blju_mainbuyer + "' and Mesg_Sample='" + blju_sample + "' ";
                        stData.GetExecuteNonQry(qry);
                        ...
                        if (mode == "add")
                        {
                            Response.Redirect("Order_abl.aspx?mode=add");
                        }
                        else
                        {
                            Response.Redirect("Order_abl.aspx");
                        }
                    }
                    else
                    {
    ...
    private void BindDataModify(string blju_date, string blju_times, string blju_mainbuyer, string blju_sample, string blju_line)
    {
        ...
        qry = " SELECT (select kure_sangho from gblKURE where kure_code = a.Bjhd_MainBuyer) as Bjhd_MainBuyerNm,* FROM ablBJHD a where bjhd_date = '" + blju_date + "' and bjhd_times = '" + blju_times + "' and bjhd_mainbuyer = '" + blju_mainbuyer + "' and bjhd_sample = '" + blju_sample + "' ";
        ...
        qry = " SELECT isnull((select isnull(Dnga_MainName,'') + ', ' + isnull(Dnga_SubName,'') + ', ' + isnull(Dnga_SpecColor,'') from ablDNGA where Dnga_StyleNox=a.Blju_StyleNox),'') as Blju_StyleName,* FROM ablBLJU a where blju_date = '" + blju_date + "' and blju_times = '" + blju_times + "' and blju_mainbuyer = '" + blju_mainbuyer + "' and blju_sample = '" + blju_sample + "' and blju_line = '" + blju_line + "' ";
        ...
        if (dsD.Tables[0].Rows.Count > 0)
        {
            ...
            qry = " select * from ablBLJU where Blju_Date = '" + dr["blju_date"].ToString() + "' and Blju_Times = '" + dr["blju_times"].ToString() + "' and Blju_MainBuyer = '" + dr["blju_mainbuyer"].ToString() + "' and Blju_StyleNox = '" + dr["Blju_StyleNox"].ToString() + "' ";
            DataSet dsC = stData.GetDataSet(qry);
            ...
            qry = " select isnull(Dnga_BoxQty,0) as Dnga_BoxQty,isnull(Dnga_JustPrice,0) as Dnga_JustPrice,isnull(Dnga_LowPrice,0) as Dnga_LowPrice from ablDNGA where Dnga_StyleNox = '" + dr["Blju_StyleNox"].ToString() + "' ";
            DataSet ds = stData.GetDataSet(qry);
            ...
            this.txtOrder13.Text = GetAmountFormat(dr["Blju_Qty13"]);
            this.txtOrder14.Text = GetAmountFormat(dr["Blju_Qty14"]);
            this.txtOrder15.Text = GetAmountFormat(dr["Blju_Qty15"]);
            this.txtOrder16.Text = GetAmountFormat(dr["Blju_Qty16"]);
            this.txtOrder17.Text = GetAmountFormat(dr["Blju_Qty17"]);
            ...
            // 이미지 확대 오픈
            qry = "SELECT * FROM ablJEPG WHERE Jepg_StyleNox='" + blju_style + "'";
    ...
    private void JegoSet(string blju_style, string mode, string date, string time, string kure, string line)
    {
        ...
        this.txtJego13.Text = JegoSet[13];
        this.txtJego14.Text = JegoSet[14];
        this.txtJego15.Text = JegoSet[15];
        this.txtJego16.Text = JegoSet[16];
        this.txtJego17.Text = JegoSet[17];
    ...
    private string JegoOverCheck(string mode)
    {
        ...
        string qry = " select * from View_ablJEGO_Summary where Jego_StyleNox = '" + product + "' ";
        ...
        if (mode == "mod") // 수정일 경우에는 재고 <= (재고 + 현재 데이터 주문량)
        {
            ...
            qry += " FROM ablBLJU a where blju_stylenox = '" + product + "' and blju_date = '" + date + "' and blju_times = '" + time + "' and blju_mainbuyer = '" + kure + "' and blju_line = '" + line + "' ";
            DataSet ds = stData.GetDataSet(qry);
    ...
    private void SetJustPrice(string blju_buyer, string blju_style)
    {
        ...
        if (dsK.Tables[0].Rows.Count > 0)
        {
            ...
            qry = " SELECT * from ablDNGA where dnga_stylenox = '" + blju_style + "' ";
            DataSet dsD = stData.GetDataSet(qry);
        ...
        */
        ...
        this.hidDnga13.Value = (price13).ToString();
        this.hidDnga14.Value = (price14).ToString();
        this.hidDnga15.Value = (price15).ToString();
        this.hidDnga16.Value = (price16).ToString();
        this.hidDnga17.Value = (price17).ToString();
    }
    ...
    private void SetHeadAmount(string blju_date, string blju_times, string blju_mainbuyer, string blju_sample)
    {
        ...
        */
        ...
        qry = " select isnull(sum(Blju_JustAmount),0) FROM ablBLJU a where blju_date = '" + blju_date + "' and blju_times = '" + blju_times + "' and blju_mainbuyer = '" + blju_mainbuyer + "' and blju_sample = '" + blju_sample + "' ";
        ...
        qry = " UPDATE ablBJHD SET Bjhd_NetAmount = '" + net.ToString() + "',Bjhd_VatAmount = '" + vat.ToString() + "',Bjhd_HapAmount = '" + hap.ToString() + "',Bjhd_ModifyDate=getDate(),Bjhd_ModifySaWon = '" + MemberData.GetLoginSID("LoginID") + "' where bjhd_date = '" + blju_date + "' and bjhd_times = '" + blju_times + "' and bjhd_mainbuyer = '" + blju_mainbuyer + "' and bjhd_sample = '" + blju_sample + "' ";
        stData.GetExecuteNonQry(qry);
    ...
    {
        string qry = " select * FROM ablBJHD where bjhd_date = '" + blju_date + "' and bjhd_times = '" + blju_times + "' and bjhd_mainbuyer = '" + blju_mainbuyer + "' and bjhd_sample = '" + blju_sample + "' ";
        DataSet ds = stData.GetDataSet(qry);
    ...
    private void WriteDefault()
    {
        ...
        this.txtOrder13.Text = "";
        this.txtOrder14.Text = "";
        this.txtOrder15.Text = "";
        this.txtOrder16.Text = "";
        this.txtOrder17.Text = "";
        ...
        string qry = " select * from ablBJHD where Bjhd_Bonsa_Check in('0') and Bjhd_Date = '" + date + "' and Bjhd_Times = '" + time + "' and Bjhd_MainBuyer = '" + kure + "' order by Bjhd_Times desc ";
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            qry = " select * from ablBLJU where Blju_Date = '" + date + "' and Blju_Times = '" + time + "' and Blju_MainBuyer = '" + kure + "' ";
            DataSet dsC = stData.GetDataSet(qry);
    ...
    protected void btnWrite_Click(object sender, EventArgs e)
    {
        ...
        /*
          ...
          사이즈별 재고량과 주문량을 다시 한 번 비교하여 모든 사이즈가 재고량 범위에 있으면 리스트에 추가하고,
            ablDNGA에서 단가를 가져와 합계수량 * 단가를 적용하고
            ablBJHD, ablBLJU에 각각 INSERT한다
        ...
        else
        {
            ...
            if (jegoOverChk == "")
            {
                ...
                order.Dnga13 = this.hidDnga13.Value;
                order.Dnga14 = this.hidDnga14.Value;
                order.Dnga15 = this.hidDnga15.Value;
                order.Dnga16 = this.hidDnga16.Value;
                order.Dnga17 = this.hidDnga17.Value;
    ...
    protected void lnkComplete_Click(object sender, EventArgs e)
    {
        ...
        string qry = " select * from ablBLJU where Blju_Date = '" + date + "' and Blju_Times = '" + time + "' and Blju_MainBuyer = '" + kure + "' ";
        DataSet dsC = stData.GetDataSet(qry);
        ...
        {
            string nowBonsaCheck = StCommon.GetBonsaCheck("abl", kure, date, time);
            if (nowBonsaCheck != "" && nowBonsaCheck != "0")
            {
                string bonsaCheckMsg = StCommon.MessageBonsaCheck(nowBonsaCheck);
            ...
            else
            {
                ...
                // 주문완료 후 종료  ablBJHD테이블 Bjhd_Bonsa_Check 필드에 '1': 주문완료
                qry = " update ablBJHD set Bjhd_Bonsa_Check = '1',Bjhd_BaeSong = '" + baesong + "',Bjhd_BaeSongName = '" + baesongname + "',Bjhd_Remark = '" + etc + "',Bjhd_ModifyDate=getDate(),Bjhd_ModifySaWon = '" + MemberData.GetLoginSID("LoginID") + "' " + whereQry;
                stData.GetExecuteNonQry(qry);

                whis.InsertWork("발주", "주문완료", qry);

                //BindDataList();
                Response.Redirect("/Page/Order_abl.aspx");
            }
        }
    }
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        if (FileFactory.CheckImageFile(this.fu.FileName))
        {
            ...
            string qry = " insert into ablJEPG(Jepg_StyleNox,Jepg_Image) values (@Jepg_StyleNox,@Jepg_Image); ";
    ...
    private void JegoEnable()
    {
        ...
        double jego13 = StCommon.ToDouble(this.txtJego13.Text, 0);
        double jego14 = StCommon.ToDouble(this.txtJego14.Text, 0);
        double jego15 = StCommon.ToDouble(this.txtJego15.Text, 0);
        double jego16 = StCommon.ToDouble(this.txtJego16.Text, 0);
        double jego17 = StCommon.ToDouble(this.txtJego17.Text, 0);
        ...
        if (jego13 == 0)
            this.txtOrder13.Enabled = false;
        if (jego14 == 0)
            this.txtOrder14.Enabled = false;
        if (jego15 == 0)
            this.txtOrder15.Enabled = false;
        if (jego16 == 0)
            this.txtOrder16.Enabled = false;
        if (jego17 == 0)
            this.txtOrder17.Enabled = false;
    }

    private void SetProduct(string product)
    {
        string qry = " select Dnga_MainName, Dnga_SubName, Dnga_SpecColor from ablDNGA where Dnga_StyleNox='" + product + "' ";
        DataSet dsD = stData.GetDataSet(qry);
    ...
    private void EnableButton(bool isEnable)
    {
        ...
        this.txtOrder13.Enabled = isEnable;
        this.txtOrder14.Enabled = isEnable;
        this.txtOrder15.Enabled = isEnable;
        this.txtOrder16.Enabled = isEnable;
        this.txtOrder17.Enabled = isEnable;
    ...
    private void BindList()
    {
        ...
        string qry = " SELECT (select kure_sangho from gblKURE where kure_code = Bjhd_MainBuyer) as Bjhd_MainBuyerNm,(select count(*) ablMESG where Mesg_Date = Bjhd_Date AND Mesg_Times = Bjhd_Times AND Mesg_MainBuyer = Bjhd_MainBuyer AND Mesg_Sample = Bjhd_Sample) msgCnt,*,ISNULL(SQL_BonSaReadOk,0) as BonSaReadOk,ISNULL(SQL_DaeRiReadOk,0) as DaeRiReadOk ";
        qry += " , LTRIM(STUFF((select ',' + Blju_StyleNox from ablBLJU where isnull(Blju_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Blju_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Blju_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Blju_Sample,'') = isnull(a.Bjhd_Sample,'') FOR XML PATH('')), 1, 1, '')) as Blju_StyleNox ";
        qry += " , (select Bjhd0_Name from ablBJHD0 where isnull(Bjhd0_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd0_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd0_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd0_Sample,'') = isnull(a.Bjhd_Sample,'')) as Bjhd0_Name ";
        qry += " , (select Bjhd0_Name_Send from ablBJHD0 where isnull(Bjhd0_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd0_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd0_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd0_Sample,'') = isnull(a.Bjhd_Sample,'')) as Bjhd0_Name_Send ";
        qry += " FROM ablBJHD a LEFT OUTER JOIN (SELECT Mesg_Date,Mesg_Times,Mesg_MainBuyer,Mesg_Sample,SUM(case when Mesg_BonSa_Daeri = '1' then Mesg_BonSaReadOk else 0 end) AS SQL_BonSaReadOk,SUM(case when Mesg_BonSa_Daeri = '0' then Mesg_DaeRiReadOk else 0 end) AS SQL_DaeRiReadOk FROM ablMESG GROUP BY Mesg_Date,Mesg_Times,Mesg_MainBuyer,Mesg_Sample) t1 ";
    ...
    protected void lvList_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        qry = " select Bjhd_Bonsa_Check FROM ablBJHD where bjhd_date = '" + blju_date + "' and bjhd_times = '" + blju_times + "' and bjhd_mainbuyer = '" + blju_mainbuyer + "' and bjhd_sample='" + blju_sample + "' ";
        DataSet dsC = stData.GetDataSet(qry);
        ...
        switch (e.CommandName)
        {
            ...
            case "subModify":
                ...
                else
                {
                    ...
                    {
                        qry = " update ablBJHD set Bjhd_Bonsa_Check = '0' where bjhd_date = '" + blju_date + "' and bjhd_times = '" + blju_times + "' and bjhd_mainbuyer = '" + blju_mainbuyer + "' and bjhd_sample='" + blju_sample + "' ";
                        stData.GetExecuteNonQry(qry);
    ...
    {
        string qry = " select Dnga_MainName, Dnga_SubName, Dnga_SpecColor from ablDNGA where Dnga_StyleNox='" + this.txtProductList.Text + "' ";
        DataSet dsD = stData.GetDataSet(qry);
    ...
    {
        ((Literal)this.lvView.FindControl("ltlSize1")).Text = st.SizeName1;
        ((Literal)this.lvView.FindControl("ltlSize2")).Text = st.SizeName2;
        ((Literal)this.lvView.FindControl("ltlSize3")).Text = st.SizeName3;
        ((Literal)this.lvView.FindControl("ltlSize4")).Text = st.SizeName4;
        ((Literal)this.lvView.FindControl("ltlSize5")).Text = st.SizeName5;
        ((Literal)this.lvView.FindControl("ltlSize6")).Text = st.SizeName6;
        ((Literal)this.lvView.FindControl("ltlSize7")).Text = st.SizeName7;
        ((Literal)this.lvView.FindControl("ltlSize8")).Text = st.SizeName8;
        ((Literal)this.lvView.FindControl("ltlSize9")).Text = st.SizeName9;
        ((Literal)this.lvView.FindControl("ltlSize10")).Text = st.SizeName10;
        ((Literal)this.lvView.FindControl("ltlSize11")).Text = st.SizeName11;
        ((Literal)this.lvView.FindControl("ltlSize12")).Text = st.SizeName12;
        ((Literal)this.lvView.FindControl("ltlSize13")).Text = st.SizeName13;
        ((Literal)this.lvView.FindControl("ltlSize14")).Text = st.SizeName14;
        ((Literal)this.lvView.FindControl("ltlSize15")).Text = st.SizeName15;
        ((Literal)this.lvView.FindControl("ltlSize16")).Text = st.SizeName16;
        ((Literal)this.lvView.FindControl("ltlSize17")).Text = st.SizeName17;
    }

    private void BindView(string bljudate, string bljutime, string kurecode, string blju_sample)
    {
        // 당일 거래처의 발주내역이 있으면 가져올것(단, 본사처리가 안된 상태인 ablBJHD테이블 Bjhd_Bonsa_Check 필드가 '0', '1'인 것만)
        string whereQry = "";
        whereQry = StCommon.MakeSearchQry("Bjhd_Times", bljutime, "S", whereQry);
        ...
        qry += " ,(select Bjhd2_SongJangNox from ablBJHD2 where isnull(Bjhd2_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd2_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd2_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd2_Sample,'') = isnull(a.Bjhd_Sample,'')) as Bjhd2_SongJangNox ";
        //qry += " ,(select top 1 Bjhd3_SongJangNox from ablBJHD3 where isnull(Bjhd3_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd3_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd3_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd3_Sample,'') = isnull(a.Bjhd_Sample,'') order by Bjhd3_Seqx) as Bjhd3_SongJangNox ";
        qry += " ,* from ablBJHD a ";
    ...
    private void BindViewList()
    {
        ...
        string qry = " SELECT isnull((select isnull(Dnga_SubName,'') + ', ' + isnull(Dnga_SpecColor,'') from ablDNGA where Dnga_StyleNox=a.Blju_StyleNox),'') as Blju_StyleName,* FROM ablBLJU a " + whereQry + " ORDER BY Blju_Line ";
        ...
        try
        {
            ((Literal)this.lvView.FindControl("ltlSumTotal")).Text = GetAmountFormat(sum1 + sum2 + sum3 + sum4 + sum5 + sum6 + sum7 + sum8 + sum9 + sum10 + sum11 + sum12 + sum13 + sum14 + sum15 + sum16 + sum17);
        }
        catch { }
