...
public partial class Page_JegoUpload_tbl : System.Web.UI.Page
{
    ...
    private void BindExcelData(string fullPathFile, string extFileName)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < dtSheets.Rows.Count; i++)
            {
                ...
                if (sheetNames[i] == "가로양식(의류용)" || sheetNames[i] == "세로양식(의류용)")
                {
            ...
            if (firstSheet == "가로양식(의류용)")
            {
                ...
                for (int i = 1; i < dt.Rows.Count; i++)
                {
                    ...
                    if (dr[0].ToString() != "")
                    {
                        ...
                        sb.Append(" INSERT INTO " + preVal + "JEGOEXCEL_H (Jego_Num, Jego_MainBuyer,Jego_StyleNox,Jego_Qty01,Jego_Qty02,Jego_Qty03,Jego_Qty04,Jego_Qty05,Jego_Qty06,Jego_Qty07,Jego_Qty08,Jego_Qty09,Jego_Qty10,Jego_Qty11,Jego_Qty12,Jego_QtyTotal) ");
                        sb.Append(" VALUES('" + (i + 2) + "','" + MemberData.GetLoginSID("KureCode") + "','" + dr[0] + "','" + GetQtyNumber(dr[1]) + "','" + GetQtyNumber(dr[2]) + "','" + GetQtyNumber(dr[3]) + "','" + GetQtyNumber(dr[4]) + "','" + GetQtyNumber(dr[5]) + "','" + GetQtyNumber(dr[6]) + "','" + GetQtyNumber(dr[7]) + "','" + GetQtyNumber(dr[8]) + "','" + GetQtyNumber(dr[9]) + "','" + GetQtyNumber(dr[10]) + "','" + GetQtyNumber(dr[11]) + "','" + GetQtyNumber(dr[12]) + "','" + GetQtyNumber(dr[13]) + "') ");
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    double chkSum = StCommon.ToDouble(dr["Jego_Qty01"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty02"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty03"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty04"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty05"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty06"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty07"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty08"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty09"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty10"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty11"].ToString(), 0) + StCommon.ToDouble(dr["Jego_Qty12"].ToString(), 0);
            ...
            {
                StJavaScript js = new StJavaScript(this.Page, false, true);
                js.WriteJavascript("showMessageToolTip('" + btnOther.ClientID + "', '시트명을 `가로양식(의류용)`이나 `세로양식(의류용)`으로 해주세요. 샘플 양식을 다운받아서 사용해주세요.');");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (this.hidGubun.Value == "가로양식(의류용)")
        {
        ...
        else if (this.hidGubun.Value == "세로양식(의류용)")
        {
            ...
            // 총계 업데이트
            qry = " UPDATE " + preVal + "JEGO SET Jego_QtyTotal = (isnull(jego_qty01,0)+isnull(jego_qty02,0)+isnull(jego_qty03,0)+isnull(jego_qty04,0)+isnull(jego_qty05,0)+isnull(jego_qty06,0)+isnull(jego_qty07,0)+isnull(jego_qty08,0)+isnull(jego_qty09,0)+isnull(jego_qty10,0)+isnull(jego_qty11,0)+isnull(jego_qty12,0)) where Jego_MainBuyer='" + MemberData.GetLoginSID("KureCode") + "' ";
            stData.GetExecuteNonQry(qry);
