...
public partial class Page_Order_tbl : System.Web.UI.Page
{
    ...
    private void BindView(string bljudate, string bljutime, string kurecode, string blju_sample)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            qry = " select * from tblBJHD3 " +
                " inner join tblBJHD a on isnull(Bjhd3_Date,'') = isnull(a.Bjhd_Date,'') and isnull(Bjhd3_Times,'') = isnull(a.Bjhd_Times,'') and isnull(Bjhd3_MainBuyer,'') = isnull(a.Bjhd_MainBuyer,'') and isnull(Bjhd3_Sample,'') = isnull(a.Bjhd_Sample,'') " +
